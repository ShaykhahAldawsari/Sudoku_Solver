...
def FC(puzzle):
    global solution_found, start_time, end_time, updated_cells
...
def update_neighbour_domain(puzzle, row, column, num):
    # Update the domain of the neighboring cell by removing the assigned value
    domain = get_domain(puzzle, row, column)
    domain.discard(num)
...
def restore_neighbour_domain(puzzle, updated_cells):
    for row, column in updated_cells:
        # Restore the domain of the neighboring cell by adding the value back
        domain = get_domain(puzzle, row, column)
...
#Function to restore the domains of cells after backtracking 
def restore_domains(board, updated_domains):
    for cell, domain in updated_domains.items():
        row, col = cell
        board[row][col]=0
        for num in domain:
...
for puzzle in puzzles:
    ...
    while running:
        ...
        if not solution_found:
            ...
            FC(puzzles[0])  # Solve the first puzzle in the list
            if solution_found:
